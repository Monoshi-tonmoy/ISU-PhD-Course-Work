...
class Othello(Game):
    ...
    def to_move(state):
        """Return the player to move in the given state."""
    ...
    def is_valid_move(self, row, col, player):
        ...
        for d in directions:
            r, c = row + d[0], col + d[1]
            if r < 0 or r >= 8 or c < 0 or c >= 8 or self.board[r][c] != other_player:
                continue
            r, c = r + d[0], c + d[1]
    ...
    def make_move(self, row, col, player):
        ...
        for d in directions:
            r, c = row + d[0], col + d[1]
            to_flip = []
            while r >= 0 and r < 8 and c >= 0 and c < 8 and self.board[r][c] == other_player:
                to_flip.append((r, c))
                r, c = r + d[0], c + d[1]
            if r >= 0 and r < 8 and c >= 0 and c < 8 and self.board[r][c] == player:
                for flip_row, flip_col in to_flip:
                    self.board[flip_row][flip_col] = player
        self.current_player = other_player if self.get_valid_moves(other_player) else player
    ...
    def actions(state):
        return state.get_valid_moves(state.current_player)
    ...
    def human_move(game):
        ...
        best_move_row,best_move_column = alpha_beta_cutoff_search(game ,Othello, d=3, eval_fn=heuristic)
        end=time.time()
    
        
        #print(best_move_row,best_move_column)
        return (best_move_row,best_move_column),(end-start)
    
           
    def result(state, action):
        # Returns a new state resulting from the current player making a move at the given action
        row, col = action
        new_state = Othello()
        new_state.board = [row[:] for row in state.board]
        new_state.current_player = state.current_player
        new_state.make_move(row, col, new_state.current_player)
        return new_state
    ...
    def utility(state, player):
        ...
        def heuristic(state):
        # The heuristic function used for alpha-beta search
        # Here, we're just using the difference between the number of black and white pieces on the board
            b_count = sum([row.count('B') for row in state.board])
            w_count = sum([row.count('W') for row in state.board])
            return b_count - w_count
    ...
    def agent2_move(game,depth):
        ...
        import time
        start=time.time()
        best_move_row,best_move_column = alpha_beta_cutoff_search(game ,Othello, depth, eval_fn=heuristic)
        end=time.time()
    ...
    def agent1_move(game,depth):
        ...
        import time
        start=time.time()
        best_move_row,best_move_column = alpha_beta_cutoff_search(game ,Othello, depth, eval_fn=weighted_heuristic)
        end=time.time()
